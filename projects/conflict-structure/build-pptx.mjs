...
const C = {
  ...
  cardLine: "33415C",
  text:     "FFFFFF",
  mute:     "C3CFE2",       // текст внутри карточек
  lead:     "D6E0F0",       // текст поверх градиента — светлее, чтобы держать контраст
  faint:    "B3C0D6",
  obj:      "6BBBFF",       // электрический голубой
  subj:     "FF9166",       // тёплый коралл
  gold:     "FFC85C",       // третий акцент для выделений
  objCard:  "12294A",
  objLine:  "3E7FC4",
  subjCard: "3A1E18",
  subjLine: "C4603E",
...
/** Фоновые картинки по разделам (генерируются make-backgrounds.mjs) */
const BG = {
  cover:   "assets/bg/cover.jpg",
  obj:     "assets/bg/obj.jpg",
  subj:    "assets/bg/subj.jpg",
  neutral: "assets/bg/neutral.jpg",
};

/** Полупрозрачность карточек — сквозь них просвечивает градиент */
const CARD_ALPHA = 18;
...
/* ---------------------------------------------------------------
   ХЕЛПЕРЫ
   --------------------------------------------------------------- */
function newSlide(notes, bg = BG.neutral) {
...
function lead(s, runs, y = 1.92, h = 0.62, size = 13.5) {
  s.addText(runs, {
    x: M, y, w: CW, h,
    fontFace: F.body, fontSize: size, color: C.lead,
    isTextBox: true, margin: 0, valign: "top",
  });
}
...
function card(s, { x, y, w, h, tone = "plain" }) {
  ...
  s.addShape(pres.ShapeType.roundRect, {
    ...
    fill: { color: fill, transparency: CARD_ALPHA },
   ...
   =============================================================== */
{
  ...
  s.addText([
    ...
    x: M, y: 3.18, w: 7.6, h: 0.7,
    fontFace: F.body, fontSize: 15, color: C.lead,
    isTextBox: true, margin: 0, valign: "top",
  });

  s.addShape(pres.ShapeType.rect, {
    x: M, y: 4.3, w: 1.2, h: 0.02, fill: { color: C.cardLine }, line: { color: C.cardLine, width: 0 },
  });

  s.addText("Выполнили студентки группы 8.536 зу", {
    x: M, y: 4.58, w: 6, h: 0.3,
    fontFace: F.body, fontSize: 11, color: C.faint,
    isTextBox: true, margin: 0, valign: "top",
  });
  s.addText("Кудякова Дарья", {
    x: M, y: 4.98, w: 3, h: 0.32,
    fontFace: F.body, fontSize: 14, bold: true, color: C.text,
    isTextBox: true, margin: 0, valign: "top",
  });
  s.addText("Бороздова Валерия", {
    x: M + 3.1, y: 4.98, w: 3, h: 0.32,
    fontFace: F.body, fontSize: 14, bold: true, color: C.text,
    isTextBox: true, margin: 0, valign: "top",
  });
}
   ...
   =============================================================== */
{
  ...
  items.forEach(([num, head, body], i) => {
    const x = M + i * (cw + gap);
    card(s, { x, y: 3.0, w: cw, h: 2.65, tone: "obj" });
    badge(s, { x: x + 0.24, y: 3.28, label: num, tone: "obj", d: 0.4 });
    cardText(s, {
      x: x + 0.24, y: 3.9, w: cw - 0.48,
...
/* ===============================================================
   ...
   =============================================================== */
{
  const s = newSlide("Спасибо за внимание. Готовы ответить на вопросы.", BG.cover);
